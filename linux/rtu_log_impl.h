...
#define RTU_LOG_ERROR(str, status)                                             \
    ...
    {                                                                          \
        logE(                                                                  \
            "%s [%s|%s|U:%d|E:%d]", str, rtu_state_str(status.bits.prev),      \
            rtu_state_str(status.bits.curr), status.bits.updated,              \
            status.bits.error);                                                \
    } while (0)
#define RTU_LOG_EVENT(str, status)                                             \
    ...
    {                                                                          \
        logD(                                                                  \
            "%s [%s|%s|E:%d]", str, rtu_state_str(status.bits.prev),           \
            rtu_state_str(status.bits.curr), status.bits.error);               \
    } while (0)
