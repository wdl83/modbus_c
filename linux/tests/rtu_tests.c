...
static const speed_t supported_rates[]
...
static char *master_read(struct TestFixture *tf, char *begin, const char *end)
{
...
static const char *
master_write(struct TestFixture *tf, const char *begin, const char *end)
{
    const int tmax_ms = calc_tmax_ms(tf->config->rate, (size_t)(end - begin));
...
static void *async_run_rtu(void *user_data)
{
    assert(user_data);
    ...
    modbus_rtu_run(
        config->dev, config->rate, config->timeout_1t5_us,
        config->timeout_3t5_us, config->pdu_cb,
        (uintptr_t)(&config->memory_impl), &config->event);
    return NULL;
}

static void serial_init(struct TestFixture *tf)
{
    // hardware test
    if (is_hw_test(tf))
    {
...
static void serial_config(
    tty_dev_t *master, tty_dev_t *slave, speed_t rate, parity_t parity)
{
...
static void serial_deinit(tty_dev_t *master, tty_dev_t *slave)
{
    tty_deinit(slave);
    tty_deinit(master);
}
...
UTEST_I_SETUP(TestFixture)
{
    ...
    logD(
        "%d %dbps 1.5t %dus 3.5t %dus", (int)utest_index, tty_bps(rate),
    ...
    CHECK_ERRNO(
        0 == pthread_create(&tf->runner, NULL, async_run_rtu, &tf->rtu_config));

    // wait for RTU thread to start
    while (!tf->rtu_config.sync.rtu_started)
        pthread_cond_wait(
            &tf->rtu_config.sync.cond, &tf->rtu_config.sync.mutex);

    pthread_mutex_unlock(&tf->rtu_config.sync.mutex);
done:
    // time required for RTU to transition from INIT to IDLE state (at least
    // 10ms)
...
UTEST_I(TestFixture, read_bytes_21, 7)
{
...

    {
        ...
        const char *const req_end = make_request_rd_bytes(
            tf->config->rtu_addr, WORD_TO_MEM_ADDR(RTU_MEMORY_ADDR), num, req,
            sizeof(req));
        ASSERT_TRUE(req_end);
...

    {
        ...
        const char *const rx_buf_end
            = master_read(tf, rx_buf, rx_buf + length_of(rx_buf));
        const modbus_rtu_rd_bytes_reply_t *rep
            = parse_reply_rd_bytes(rx_buf, rx_buf_end - rx_buf);
...
UTEST_I(TestFixture, write_bytes_func_STR, 7)
{
...

    {
        char req[ADU_CAPACITY];
        memset(req, 0, sizeof(req));
        const char *const req_end = make_request_wr_bytes(
            tf->config->rtu_addr, WORD_TO_MEM_ADDR(RTU_MEMORY_ADDR),
            (const uint8_t *)message, length_of(message), req, sizeof(req));
        ASSERT_TRUE(req_end);
        EXPECT_EQ(req_end, master_write(tf, req, req_end));
    }

    {
        ...
        memset(rx_buf, 0, sizeof(rx_buf));
        const char *const rx_buf_end
            = master_read(tf, rx_buf, rx_buf + sizeof(rx_buf));
        const modbus_rtu_wr_bytes_reply_t *rep
            = parse_reply_wr_bytes(rx_buf, rx_buf_end - rx_buf);
...
UTEST(rtu_tests, wr_bytes_request)
{
    ...
    const char *const reqB_end = make_request_wr_bytes(
        0xAB, WORD_TO_MEM_ADDR(0x1234), (const uint8_t *)msg, length_of(msg),
        reqB, sizeof(reqB));
...
UTEST_I(TestFixture, read_holding_registers_33, 7)
{
    ...
    }
    {
        ...
        if (!is_hw_test(tf))
        {
            /* rtu memory fill pattern [0, 1 ... 255, 0, 1 .. 255, ..] (every
             * byte) however rtu_memory implementation is done for 8-bit uC so
             * every byte of addressable memory is treated as register with most
             * significant byte set to 0x00 */
...
UTEST_I(TestFixture, read_wr_register_func_0x00CD_offset_50, 7)
{
...

    {
        ...
        const char *const tx_end = make_request_wr_register(
            tf->config->rtu_addr, WORD_TO_MEM_ADDR(RTU_MEMORY_ADDR + 50),
            (data16_t){.high = 0, .low = 0xCD}, tx_buf, sizeof(tx_buf));
...
UTEST_I(TestFixture, master_write_read_bytes, 7)
{
    ...
    const uint8_t *const tx_end = rtu_master_wr_bytes(
        &impl, tf->config->rtu_addr, WORD_TO_MEM_ADDR(RTU_MEMORY_ADDR),
    ...
    uint8_t *const rx_end = rtu_master_rd_bytes(
        &impl, tf->config->rtu_addr, WORD_TO_MEM_ADDR(RTU_MEMORY_ADDR),
...
UTEST_I(TestFixture, master_write_read_holding_registers, 7)
{
    ...
    const data16_t tx_data[] = {WORD_TO_DATA16(0x0012), WORD_TO_DATA16(0x0034),
                                WORD_TO_DATA16(0x0056), WORD_TO_DATA16(0x0078),
                                WORD_TO_DATA16(0x009A), WORD_TO_DATA16(0x00BC),
                                WORD_TO_DATA16(0x00DE), WORD_TO_DATA16(0x00F0)};
    ...
    const data16_t *const tx_data_end = rtu_master_wr_registers(
        &impl, tf->config->rtu_addr, WORD_TO_MEM_ADDR(RTU_MEMORY_ADDR),
    ...
    data16_t *const rx_data_end = rtu_master_rd_holding_registers(
        &impl, tf->config->rtu_addr, WORD_TO_MEM_ADDR(RTU_MEMORY_ADDR),
...
static speed_t parse_speed(const char *str)
{
...
void help(const char *argv0, const char *message)
{
    ...
    for (size_t i = 0u; i < length_of(supported_rates); ++i)
    {
        printf(
            ...
            tty_rate_str(supported_rates[i]), calc_1t5_us(supported_rates[i]),
            calc_3t5_us(supported_rates[i]));
    }
...
int main(int argc, const char *const argv[])
{
    ...
    for (int c; !fallback
         && -1 != (c = getopt(argc, (char **)argv, "D:T:a:d:hp:r:t:?"));)
    {
        switch (c)
        {
        case 'D': g_test_config->debug_size = optarg ? atoi(optarg) : 0; break;
        case 'T':
            g_test_config->timeout_3t5_us = optarg ? atoi(optarg) : -1;
            break;
        case 'a': g_test_config->rtu_addr = optarg ? atoi(optarg) : -1; break;
        case 'd':
            g_test_config->dev_path        = optarg ? strdup(optarg) : NULL;
            g_test_config->timeout_exec_ms = TIMEOUT_EXEC_MS;
            break;
        case 'h': help(argv[0], NULL); break;
        case 'p': g_test_config->parity = parse_parity(optarg); break;
        case 'r': g_test_config->rate = parse_speed(optarg); break;
        case 't':
