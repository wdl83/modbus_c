...
static void init(tty_dev_t *master, tty_dev_t *slave)
{
    const int debug_size
        = LOG_LEVEL_DEBUG > current_log_level() ? 0 : DEBUG_SIZE;
    tty_pair_t pair;
...
static void
config(tty_dev_t *master, tty_dev_t *slave, speed_t rate, parity_t parity)
{
...
static void deinit(tty_dev_t *master, tty_dev_t *slave)
{
    tty_deinit(slave);
    tty_deinit(master);
}
...
static void *async_read(void *user_data)
{
    CHECK(user_data);
...
UTEST(tty_dev, async_read_and_write)
{
    ...
    const int timeout     = 100;
    async_data_t data     = {.dev = &slave, .timeout = timeout};
    pthread_t receiver;
...
static void *async_echo(void *user_data)
{
    CHECK(user_data);
...
UTEST(tty_dev, async_echo)
{
    ...
    async_data_t data       = {.dev = &slave, .timeout = 20};

    pthread_t receiver;
    CHECK_ERRNO(0 == pthread_create(&receiver, NULL, async_echo, &data));

    // send 1 word at a time
    for (const char *w_begin = message, *w_end = message;
         w_begin != message_end; w_begin = w_end)
    {
        while (w_begin != message_end && isspace(*w_begin))
            ++w_begin;
        w_end = w_begin;
        while (w_end != message_end && !isspace(*w_end))
            ++w_end;
        ...
        for (char *curr = rx_buf; len != (size_t)(curr - rx_buf); next = curr)
            curr = tty_read(
                &master, curr, rx_buf + length_of(rx_buf), timeout, NULL);
...
static void handle_USR1(int sig_no)
{
...
UTEST(dev_tty_t, read_interrupted)
{
    ...
    async_data_t data = {.dev = &slave, .timeout = 1000};
    pthread_t receiver;
    buf_t *recv_buf = NULL;
