...
static char *
implace_crc_impl(char *const begin, char *const end, const size_t max_size)
{
    if ((size_t)(end - begin) + sizeof(crc_t) > max_size) return NULL;

    const crc_t crc
        = modbus_rtu_calc_crc((const uint8_t *)begin, (const uint8_t *)end);
    char *curr = end;
    ...
    const size_t max_size)
{
    ...
    const char req[] = {slave_addr,   FCODE_RD_COILS, mem_addr.high,
                        mem_addr.low, UINT8_C(0),     count};
    ...
    const size_t max_size)
{
    ...
    if (sizeof(modbus_rtu_rd_holding_registers_request_t) > max_size)
        return NULL;
...
const modbus_rtu_rd_holding_registers_reply_t *
parse_reply_rd_holding_registers(const void *adu, size_t adu_size)
{
    ...
    const size_t max_size)
{
    ...
    const char req[] = {slave_addr,   FCODE_WR_COIL, mem_addr.high,
                        mem_addr.low, data,          UINT8_C(0)};
...
const modbus_rtu_wr_register_reply_t *
parse_reply_wr_register(const void *adu, size_t adu_size)
{
    ...
    const size_t max_size)
{
    ...
    const size_t expected_size
        = sizeof(modbus_rtu_wr_registers_request_header_t) + data_size
        + sizeof(crc_t);
...
{
    const size_t expected_size = sizeof(addr_t) + sizeof(fcode_t)
