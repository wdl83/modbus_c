...
static uint8_t *wr16(uint8_t *dst, uint16_t data)
{
    *dst++ = data >> 8;
...
static uint8_t *except(fcode_t fcode, ecode_t ecode, uint8_t *reply)
{
    ...
    uint8_t *reply)
{
    ...
    RETURN_EXCEPTION_IF(
        request_size > end - begin, FCODE_WR_BYTES, ECODE_FORMAT_ERROR, reply);

    const uint16_t addr = rd16(curr);
    curr += sizeof(addr);

    RETURN_EXCEPTION_IF_NOT(
        rtu_mem_end > addr && rtu_mem_begin <= addr, FCODE_RD_HOLDING_REGISTERS,
        ECODE_ILLEGAL_DATA_ADDRESS, reply);

    const uint16_t num = rd16(curr);
    curr += sizeof(num);

    RETURN_EXCEPTION_IF_NOT(
        0 < num && 0x7E > num, FCODE_RD_HOLDING_REGISTERS,
        ECODE_ILLEGAL_DATA_VALUE, reply);

    RETURN_EXCEPTION_IF_NOT(
        rtu_mem_end >= addr + num, FCODE_RD_HOLDING_REGISTERS,
        ECODE_ILLEGAL_DATA_ADDRESS, reply);
    ...
    uint8_t *reply)
{
    ...
    RETURN_EXCEPTION_IF_NOT(
        rtu_mem_end > addr, FCODE_WR_REGISTER, ECODE_ILLEGAL_DATA_ADDRESS,
        reply);
    ...
    RETURN_EXCEPTION_IF(
        UINT16_C(0xFF00) & data, FCODE_WR_REGISTER, ECODE_ILLEGAL_DATA_VALUE,
        reply);
    ...
    uint8_t *reply)
{
    ...
    RETURN_EXCEPTION_IF_NOT(
        rtu_mem_end > addr, FCODE_WR_REGISTERS, ECODE_ILLEGAL_DATA_ADDRESS,
        reply);
    ...
    RETURN_EXCEPTION_IF_NOT(
        0 < num && 0x7C > num, FCODE_WR_REGISTERS, ECODE_ILLEGAL_DATA_VALUE,
        reply);
    ...
    RETURN_EXCEPTION_IF(
        byte_count != (num << 1), FCODE_WR_REGISTERS, ECODE_ILLEGAL_DATA_VALUE,
        reply);

    RETURN_EXCEPTION_IF(
        byte_count != end - curr, FCODE_WR_REGISTERS, ECODE_FORMAT_ERROR,
        reply);
    ...
    for (; addr_begin != addr_end; ++addr_begin)
    {
        ...
        RETURN_EXCEPTION_IF(
            UINT16_C(0xFF00) & data, FCODE_WR_REGISTERS,
            ECODE_ILLEGAL_DATA_VALUE, reply);
    ...
    uint8_t *reply)
{
    ...
    RETURN_EXCEPTION_IF_NOT(
        rtu_mem_end > addr && rtu_mem_begin <= addr, FCODE_RD_BYTES,
        ECODE_ILLEGAL_DATA_ADDRESS, reply);
    ...
    RETURN_EXCEPTION_IF_NOT(
        0 < num && 250 > num, FCODE_RD_BYTES, ECODE_ILLEGAL_DATA_VALUE, reply);

    RETURN_EXCEPTION_IF_NOT(
        rtu_mem_end >= addr + num, FCODE_RD_BYTES, ECODE_ILLEGAL_DATA_ADDRESS,
        reply);
    ...
    uint8_t *reply)
{
    ...
    RETURN_EXCEPTION_IF(
        (ptrdiff_t)request_size > end - begin, FCODE_WR_BYTES,
        ECODE_FORMAT_ERROR, reply);

    const uint16_t addr = rd16(curr);
    curr += sizeof(addr);

    RETURN_EXCEPTION_IF_NOT(
        rtu_mem_end > addr, FCODE_WR_BYTES, ECODE_ILLEGAL_DATA_ADDRESS, reply);
    ...
    /* max 249 bytes in PDU: 256 - 7 (slave_addr + fcode + addr + num + crc)*/
    RETURN_EXCEPTION_IF_NOT(
        0 < num && 250 > num, FCODE_WR_BYTES, ECODE_ILLEGAL_DATA_VALUE, reply);

    RETURN_EXCEPTION_IF(
        num != end - curr, FCODE_WR_BYTES, ECODE_ILLEGAL_DATA_VALUE, reply);
